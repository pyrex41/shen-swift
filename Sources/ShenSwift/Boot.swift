...
extension Interp {
    /// The 15 KLambda files of Mark Tarver's Shen 42 kernel (mirror
    /// pyrex41/shen-upstream @ tag s42-pristine-20260825 — see klambda/PROVENANCE.md),
    ...
    ///
    /// Two-phase load. The Shen 42 kernel has no `shen.initialise`;
    ...
    ///
    /// Since S42 the stdlib is no longer part of the kernel (there is no
    /// `stlib.kl`); it ships as lazy Shen sources under upstream `Lib/StLib`
    /// (mirror `pyrex41/shen-upstream`, tag `s42-pristine-20260825`). Those
    ...
    func installNativeOverrides() {
        ...
        // yggdrasil shakes, which run under -q). Writing bytes directly also
    ...
    /// Boots from a Yggdrasil-shaken slice — a minimal `kernel.kl` (just the
    ...
    /// builders: all defuns first, then the native overrides, then (for pre-S42
    ...
    ///
    /// NOTE: slices shaken from the Shen 42 kernel carry their own
    /// initialisation in `kTops` (no `shen.initialise`); they must be
    /// regenerated by the Yggdrasil shaker and re-checked with `bifrost --shake`.
    public func bootShaken(kernel: URL, user: URL, verbose: Bool = false) throws {
        ...
        installNativeOverrides()
        // Pre-S42 shaken slices bundled a `shen.initialise`; the refreshed
    ...
    /// Starts the interactive Shen read-eval-print loop. The Shen 42
    ...
    /// `shen.x.launcher.main` is not part of Tarver's Shen 42 kernel, so
